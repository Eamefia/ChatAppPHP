                ...
                if (isset($_FILES['image'])) {
                    ...
                    if (in_array($img_ext, $extentions) === true) {
                        ...
                        if (move_uploaded_file($tmp_name, "images/".$new_image_name)) {
                            ...
                            $query = mysqli_query($conn, "INSERT INTO users (unique_id, fname, lname, email, password, img, status)
                                VALUE ({$random_id}, '{$fname}', '{$lname}', '{$email}', '{$password}', '{$new_image_name}','{$status}')");
                            if ($query) {
